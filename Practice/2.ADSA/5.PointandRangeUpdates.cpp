...
int PointandRangeUpdates(int ind, int low, int high, int node, int val)
{
    if (low == high)
        seg[low] += val;
    else
    {
        ...
        if (node <= mid && node >= low)
            PointandRangeUpdates(2 * ind + 1, low, mid, node, val);
        else
            PointandRangeUpdates(2 * ind + 2, mid + 1, high, node, val);
